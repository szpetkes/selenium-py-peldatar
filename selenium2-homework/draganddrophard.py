"""""""""
Készíts egy Python alkalmazást ami selenium-ot használ. Indítsd el lokálisan a selenium-py-peldatar alkalmazást. A 
program töltse be a példatárból az http://localhost:9999/dragndrop2.html oldalt. A feladatod, hogy a todo oszlobpól 
átrakd az összes kártyát a doing oszlopba. 
A megoldást egy draganddrophard.py és dnd.js nevű fileban kell beadnod.
"""""""""
